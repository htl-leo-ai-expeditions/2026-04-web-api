...
- **Models good practice.** The example demonstrates server-generated fields (`id`, `createdAt`), default values (`membershipStatus`), and structured error responses with a `details` array. These are patterns students should adopt.
...
- **Sets expectations concretely.** Templates make the expected granularity visible. For example, the entity table has columns for Type, Required, Unique, and Notes, signaling that students need to think about all of these per attribute.
